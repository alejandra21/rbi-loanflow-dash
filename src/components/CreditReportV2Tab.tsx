...
export const CreditReportV2Tab = ({
  ...
  const tierChangeReason = "FICO score below 680 threshold. Auto-assigned from previous tier (Platinum/Gold in Experience Tiering) to Silver/Bronze per credit review policy.";
  ...
  const calculateTLODecision = (guarantorData: (typeof tloData)["John Doe"]) => {
    ...
    const hasActiveWithin120Months = guarantorData.backgroundCheck.liens.monthsSinceLatest !== null && guarantorData.backgroundCheck.liens.monthsSinceLatest <= 120 && guarantorData.backgroundCheck.liens.old === 0 || guarantorData.backgroundCheck.judgments.monthsSinceLatest !== null && guarantorData.backgroundCheck.judgments.monthsSinceLatest <= 120 && guarantorData.backgroundCheck.judgments.old === 0 || guarantorData.backgroundCheck.bankruptcies.monthsSinceLatest !== null && guarantorData.backgroundCheck.bankruptcies.monthsSinceLatest <= 120 && guarantorData.backgroundCheck.bankruptcies.old === 0;
      ...
      {guarantors.map(guarantor => <Card key={guarantor.name}>
          ...
          {expandedGuarantors[guarantor.name] && <CardContent className="space-y-6">
                ...
                <CollapsibleContent>
                  <div className="p-4 bg-muted/20 rounded-lg mt-3 space-y-4">
                    ...
                    <div>
                      ...
                      {!isCreditReportDateValid(guarantor.pullDate) && <div className="mt-3 p-3 bg-red-50 dark:bg-red-900/20 rounded-lg border border-red-200 dark:border-red-800">
                    ...
                    <div>
                      ...
                      {guarantor.utilization >= 50 && <div className="mt-3 p-3 bg-amber-50 dark:bg-amber-900/20 rounded-lg border border-amber-200 dark:border-amber-800">
              ...
              <Collapsible open={expandedGuarantorSections[`${guarantor.name}-tlo`]} onOpenChange={() => toggleGuarantorSection(`${guarantor.name}-tlo`)}>
                ...
                return tloResult.decision === "non_pass" ? getStatusIcon("fail") : tloResult.decision === "manual_validation" ? getStatusIcon("warn") : getStatusIcon("pass");
              })()}
              ...
              return <div className="p-4 bg-muted/20 rounded-lg mt-3 space-y-4">
                        ...
                        <div>
                          ...
                          {/* Detailed Items */}
                          {(tloGuarantorData.backgroundCheck.liens.items.length > 0 || tloGuarantorData.backgroundCheck.judgments.items.length > 0 || tloGuarantorData.backgroundCheck.bankruptcies.items.length > 0 || tloGuarantorData.backgroundCheck.foreclosures.items.length > 0) && <Collapsible open={expandedBackgroundCheckDetails[guarantor.name]} onOpenChange={() => toggleBackgroundCheckDetails(guarantor.name)} className="mt-4">
                              ...
                              <CollapsibleContent className="mt-3 space-y-3">
                                {tloGuarantorData.backgroundCheck.liens.items.length > 0 && <div className="p-3 bg-muted/30 rounded border">
                                    <p className="text-xs font-semibold mb-2">Liens Found:</p>
                                    {tloGuarantorData.backgroundCheck.liens.items.map((item, idx) => <div key={idx} className="text-xs mb-2 last:mb-0">
                                ...
                                {tloGuarantorData.backgroundCheck.judgments.items.length > 0 && <div className="p-3 bg-muted/30 rounded border">
                                    <p className="text-xs font-semibold mb-2">Judgments Found:</p>
                                    {tloGuarantorData.backgroundCheck.judgments.items.map((item: any, idx: number) => <div key={idx} className="text-xs mb-2 last:mb-0">
                                        <span className="font-medium">{item.type}</span> - {item.amount} | {item.date} |{" "}
                                        <span className={item.status === "Active" ? "text-warning" : "text-success"}>
                                          {item.status}
                                        </span>
                                ...
                                {tloGuarantorData.backgroundCheck.bankruptcies.items.length > 0 && <div className="p-3 bg-muted/30 rounded border">
                                    <p className="text-xs font-semibold mb-2">Bankruptcies Found:</p>
                                    {tloGuarantorData.backgroundCheck.bankruptcies.items.map((item, idx) => <div key={idx} className="text-xs mb-2 last:mb-0">
                                        <span className="font-medium">{item.type}</span> | Filed: {item.filingDate} |
                                        Discharged: {item.dischargeDate} |{" "}
                                        <span className="text-muted-foreground">{item.status}</span>
                                      </div>)}
                                  </div>}
                                {tloGuarantorData.backgroundCheck.foreclosures.items.length > 0 && <div className="p-3 bg-muted/30 rounded border">
                                    <p className="text-xs font-semibold mb-2">Foreclosures Found:</p>
                                    {tloGuarantorData.backgroundCheck.foreclosures.items.map((item, idx) => <div key={idx} className="text-xs mb-2 last:mb-0">
              ...
              return <div className="p-4 bg-muted/20 rounded-lg mt-3 space-y-4">
                        ...
                        {hasMatch && lexisData.matchedEntities.length > 0 && <>
                            <Separator />
                            <Collapsible open={expandedLexisNexisMatches[guarantor.name]} onOpenChange={() => toggleLexisNexisMatches(guarantor.name)}>
      ...
      <Card>
        <CardHeader className="cursor-pointer hover:bg-muted/50 transition-colors" onClick={() => toggleCard("flagDat")}>
          <CardTitle className="text-base flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              {(() => {
              const hasAnyMatches = Object.values(flagDatData).some(data => data.watchlistMatches > 0 || data.blacklistMatches > 0);
              ...
              <CollapsibleContent className="mt-3">
                <div className="border rounded-lg overflow-hidden">
                  <Table>
                    ...
                    <TableBody>
                      {flagDatDetailedResults.map((result, index) => <TableRow key={index}>
